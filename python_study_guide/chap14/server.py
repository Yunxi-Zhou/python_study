...
class Server(wx.Frame):
    def __init__(self):
        ...
        stop_sever_btn=wx.Button(pl,size=(133,40),label='停止服务')
        
        # 接到可伸缩的网格布局中
        
        fgz1.Add(start_server_btn,1,wx.TOP)
        fgz1.Add(record_btn,1,wx.TOP)
        fgz1.Add(stop_sever_btn,1,wx.TOP)
    ...
    def start_server(self,event):
        #print('Starting server button clicked')
    ...
    def do_work(self):
        ...
        while self.isOn:
            ...
            # 输出服务器的提示信息
            self.show_info_send_client('服务器通知',f'欢迎{user_name}进入聊天室！',time.strftime('%Y-%m-%d %H:%M:%S'))
            
        # 当self.isOn的值为False时，执行关闭socket对象
        self.server_socket.close()
    
    def show_info_send_client(self,data_source,data,date_time):
        # 字符串拼接操作
...
class SesstionThread(threading.Thread):
    def __init__(self,client_socket,user_name,server):
        ...
        self.server=server
        self.isOn=True # 会话的线程是否启动，当创建SessionThread对象时会话线程即启动
        
        
        
    def run(self):
        print(f'Client: {self.user_name}have connected with Server, Server start a contact thread')
        ...
        while self.isOn:
            ...
            if data=='Z-disconnect-YZ':
                self.isOn=False
            else:
                # 其他聊天信息显示给所有的客户端，包含服务器也显示
                # 调用刚才编写的方法
                self.server.show_info_send_client(self.user_name,data,time.strftime('%Y-%m-%d %H:%M:%S'))
